...
Viết tắt trong URI: `@{...}` đứng giữa URL = PR id (`triggerBody()['number']`); tiền tố `.../` = lặp lại phần URL của action HTTP ngay trước — luôn ghép lại thành concat đầy đủ theo mẫu `HTTP_GetPR` ở trên.
