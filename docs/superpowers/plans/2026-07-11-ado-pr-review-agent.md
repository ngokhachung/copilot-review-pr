...
> **Amendment 2026-07-12c:** nội dung flow spec nhúng ở đây đã được viết lại
> thành bản build-ready chi tiết (mỗi action ghi rõ loại action, từng ô điền,
> expression dán nguyên) — **nguồn chính thức duy nhất: `docs/flow-specs/review-pipeline.md`**.
> Quy tắc byte-identity giữa plan và file spec chấm dứt từ đây; bản nhúng gốc
> xem lịch sử git (commit a12e24e trở về trước). Lưu ý sửa lỗi trong bản mới:
> 3 action Initialize variable phải nằm NGOÀI Scope_Try (Power Automate không
> cho Initialize variable trong Scope) — Scope_Try chứa action 4→27.
