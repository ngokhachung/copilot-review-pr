...
- Bên trong, 2 action nối tiếp:
  - **`Compose_ThreadBody`** — loại **Compose** — build body thread bằng
    ...
    **Amendment 2026-07-17c:** đổi format `content` lần nữa — bỏ footer
    `<sub>PR Review Agent · type</sub>`, gộp ID + nội dung finding vào
    CHUNG 1 dòng, và label giờ là **markdown link kèm icon badge**. Còn 3
    dòng:
    ```
    ...
    Gợi ý solution:
    {suggestion}          ← chỉ có nếu suggestion không rỗng
    ```
    `label` + icon URL suy ra CÙNG LÚC từ `type`+`severity` (dùng badge
    tự sinh từ img.shields.io — user sẽ tự thay link thật sau nếu cần,
    sửa trực tiếp URL trong expression, mỗi label có 1 URL riêng nên chỉ
    cần replace-string): `type="bug"` hoặc (`type="rule"` và `severity="error"`)
    → `[must](https://img.shields.io/badge/must-red)`; `type="rule"` và
    `severity="warning"` → `[suggestion](https://img.shields.io/badge/suggestion-green)`;
    `type="rule"` và `severity="info"`, hoặc `type="clean"` →
    `[nits](https://img.shields.io/badge/nits-lightgrey)`; `type="arch"` →
    `[imo](https://img.shields.io/badge/imo-blue)`. Hai nhãn `imho`/`ask`
    trong bộ convention KHÔNG được bot dùng tự động — để dành cho người
    review comment tay. **Bỏ fallback literal `"BUG"`**: dòng ID giờ luôn
    lấy `rule` thật do prompt sinh ra (xem amendment 2026-07-17c ở
    `prompts/review-prompt.md` — type "bug" giờ cũng có short tag như
    `BUG-NULL-DEREF` thay vì chữ "BUG" cứng); `coalesce(..., 'BUG-OTHER')`
    chỉ là lưới an toàn phòng khi AI quên set `rule`.
    Inputs (fx, dán nguyên MỘT dòng):
    ```
    json(replace(replace(string(addProperty(addProperty(addProperty(addProperty(json('{}'), 'comments', createArray(addProperty(addProperty(addProperty(json('{}'), 'parentCommentId', 0), 'commentType', 1), 'content', concat(if(equals(item()?['type'],'bug'),'[must](https://img.shields.io/badge/must-red)',if(equals(item()?['type'],'rule'),if(equals(item()?['severity'],'error'),'[must](https://img.shields.io/badge/must-red)',if(equals(item()?['severity'],'warning'),'[suggestion](https://img.shields.io/badge/suggestion-green)','[nits](https://img.shields.io/badge/nits-lightgrey)')),if(equals(item()?['type'],'arch'),'[imo](https://img.shields.io/badge/imo-blue)','[nits](https://img.shields.io/badge/nits-lightgrey)'))), decodeUriComponent('%0A'), coalesce(item()?['rule'],'BUG-OTHER'), ': ', item()?['message'], if(empty(item()?['suggestion']),'',concat(decodeUriComponent('%0A%0A'),'Gợi ý solution:', decodeUriComponent('%0A'), item()?['suggestion'])))))), 'status', 1), 'threadContext', addProperty(addProperty(addProperty(json('{}'), 'filePath', if(startswith(item()?['file'],'/'), item()?['file'], concat('/', item()?['file']))), 'rightFileStart', addProperty(addProperty(json('{}'), 'line', max(int(coalesce(item()?['line'], 1)), 1)), 'offset', 1)), 'rightFileEnd', addProperty(addProperty(json('{}'), 'line', max(int(coalesce(item()?['line'], 1)), 1)), 'offset', 1))), 'properties', addProperty(addProperty(json('{}'), 'prvFingerprint', addProperty(addProperty(json('{}'), '$type', 'System.String'), '$value', item()?['fingerprint'])), 'prvRule', addProperty(addProperty(json('{}'), '$type', 'System.String'), '$value', coalesce(item()?['rule'], ''))))), '"prvFingerprint"', '"prv.fingerprint"'), '"prvRule"', '"prv.rule"'))
    ```
    *(Body dựng ra đúng cấu trúc: `comments[0].content` = nội dung comment
    như trên; `threadContext` neo file/dòng — line ép về số nguyên
